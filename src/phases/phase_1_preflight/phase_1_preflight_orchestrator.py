...
from phases.libraries.config.config_validator import ConfigValidator
...
class Phase1PreflightOrchestrator:
    ...
    def _step_20_validate_configuration(
    ...
    def _step_40_check_port_availability(
        self, context: Dict[str, Any]
    ) -> Dict[str, Any]:
        """
        ...
        try:
            ...
            if "ports" in config_to_check:
                ports = (
                    config_to_check["ports"]
                    if isinstance(config_to_check["ports"], list)
                    else [config_to_check["ports"]]
                )
            elif "port" in config_to_check:
                ports = [config_to_check["port"]]

            # Also check services for port definitions
            if "services" in config_to_check and isinstance(
                config_to_check["services"], dict
            ):
    ...
    def _step_60_validate_system_resources(
        self, context: Dict[str, Any]
    ) -> Dict[str, Any]:
        """
        ...
        try:
            ...
            all_sufficient = (
                mem_status.sufficient
                and disk_status.sufficient
                and cpu_status.sufficient
            )
