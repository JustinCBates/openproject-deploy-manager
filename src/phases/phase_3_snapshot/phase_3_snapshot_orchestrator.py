...
class Phase3SnapshotOrchestrator:
    ...
    def _step_10_capture_container_states(self, context: Dict[str, Any]) -> Dict[str, Any]:
        ...
        try:
            ...
            logger.info(f"    ✅ Captured {len(result.containers)} containers, "
    ...
    def _step_20_backup_configuration_files(self, context: Dict[str, Any]) -> Dict[str, Any]:
        ...
        try:
            ...
            compose_file = context.get('compose_file') or context.get('rendered_docker_compose')
            ...
            # Backup files
            logger.info(f"    💾 Backing up {len(files_to_backup)} configuration file(s)")
            ...
            logger.info(f"    ✅ Backed up {result.file_count} files ({result.total_size} bytes)")
    ...
    def _step_30_store_snapshot(self, context: Dict[str, Any]) -> Dict[str, Any]:
        ...
        try:
            ...
            # Get project name
            validated_config = context.get('validated_config')
            project_name = validated_config.get('project_name', 'unknown') if validated_config else 'unknown'
