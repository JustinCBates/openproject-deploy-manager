...
class Phase6PostDeploymentOrchestrator:
    ...
    def _step_10_report_deployment_status(
        ...
        try:
            ...
            logger.info(
                f"    {success_icon} Status: {deployment_status.get('success', False)}"
            )
            logger.info(f"    Phases: {phases.get('completed', [])} completed")
            logger.info(
                f"    Services: {services.get('deployed', 0)} deployed, "
            ...
            return {"status": "failed", "artifacts": {}, "messages": [str(e)]}

    def _step_20_log_deployment_metadata(
        self, context: Dict[str, Any]
    ) -> Dict[str, Any]:
        """
        Step 20: Log Deployment Metadata
        ...
        try:
            ...
            if success:
                ...
                if recent_entries:
                    ...
                    logger.info(
                        f"    Last entry: project={last_entry.get('project_name')}, "
    ...
    def _step_30_cleanup_temporary_files(
        self, context: Dict[str, Any]
    ) -> Dict[str, Any]:
        """
        Step 30: Cleanup Temporary Files
        ...
        try:
            ...
            template_temp_dir = (
                self.project_root / "runtime" / "phase_2_template_rendering" / "temp"
            )
            if template_temp_dir.exists():
                ...
                logger.info(
                    f"    Template cleanup: {result.files_removed} files, "
                    f"{result.space_freed_bytes} bytes freed"
                )

            # Clean up old snapshots (keep 5 most recent)
            snapshots_dir = (
                self.project_root / "runtime" / "phase_3_snapshot" / "outputs"
            )
            if snapshots_dir.exists():
                result = cleanup_handler.cleanup_old_artifacts(
                    snapshots_dir, keep_count=5, pattern="snapshot_*"
                )
                logger.info(
                    f"    Snapshot cleanup: kept 5 newest, removed {result.files_removed + result.directories_removed}"
                )
            ...
            if health_dir.exists():
                result = cleanup_handler.cleanup_old_artifacts(
                    health_dir, keep_count=5, pattern="health_report_*"
                )
                logger.info(
                    f"    Health report cleanup: kept 5 newest, removed {result.files_removed + result.directories_removed}"
                )
            ...
            return {
                "status": "success",
                "artifacts": {"cleanup_completed": True},
                "messages": [],
            }
...
def main():
    ...
    context = {
        ...
        "phase_results": {
            ...
            "phase_5_health_verification": {
                "status": "success",
                "artifacts": {"containers_healthy": 3, "endpoints_healthy": 2},
            },
        },
        "errors": [],
