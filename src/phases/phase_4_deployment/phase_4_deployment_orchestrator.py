...
class Phase4DeploymentOrchestrator:
        ...
        try:
            ...
            if not env_file_path.exists():
                return {
                    "status": "error",
                    "artifacts": {},
                    "messages": [f"Failed to generate environment file: {env_file_path}"]
                }
            
            # Count lines (excluding comments and empty lines)
            with open(env_file_path, 'r') as f:
                lines = f.readlines()
            variable_count = sum(1 for line in lines if line.strip() and not line.strip().startswith('#'))
    ...
    def _step_20_pull_docker_images(self, context: Dict[str, Any]) -> Dict[str, Any]:
        ...
        try:
            ...
            pull_images = validated_config.get('deployment', {}).get('pull_images', True)
            ...
            already_exists = [img for img, r in results.items() if r.success and r.already_exists]
            ...
            if failed:
                return {
                    ...
                    "artifacts": {
                        "images_pulled": True,
                        "pull_results": {img: {"success": r.success, "error": r.error} for img, r in results.items()}
                    },
                    "messages": [f"Failed to pull {len(failed)} image(s): {', '.join(failed)}"]
                }
            ...
            return {
                ...
                "artifacts": {
                    "images_pulled": True,
                    "pull_results": {img: {"success": r.success, "already_exists": r.already_exists} for img, r in results.items()}
                },
        ...
        try:
            ...
            if not result.success:
                return {
                    ...
                    "messages": [f"Docker compose up failed: {result.error or result.stderr}"]
                }
    ...
    def _step_40_monitor_service_startup(self, context: Dict[str, Any]) -> Dict[str, Any]:
        ...
        try:
            ...
            if validated_config:
                startup_timeout = validated_config.get('deployment', {}).get('startup_timeout', 120)
            ...
            # Monitor startup
            logger.info(f"    ⏳ Monitoring service startup (timeout: {startup_timeout}s)...")
            ...
            if not result.success:
                ...
                for container in result.containers:
                    logger.error(f"       {container.name}: {container.state} (health: {container.health})")
            ...
            # Log success
            logger.info(f"    ✅ All services started successfully ({result.elapsed_time:.1f}s)")
            for container in result.containers:
                status_icon = "✅" if container.state == "running" else "❌"
                health_status = f" (health: {container.health})" if container.health != "none" else ""
                logger.info(f"       {status_icon} {container.name}: {container.state}{health_status}")
