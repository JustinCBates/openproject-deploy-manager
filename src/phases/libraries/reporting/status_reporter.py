...
class StatusReporter:
    ...
    def generate_report(self, context: Dict[str, Any], output_path: Optional[Path] = None) -> Dict[str, Any]:
        """
        ...
        try:
            ...
            phases_completed = [p.get('phase_id') for p in all_phases if p.get('status') == 'success']
            phases_failed = [p.get('phase_id') for p in all_phases if p.get('status') in ['error', 'failed']]
...
def main():
    ...
    test_context = {
        ...
        'phase_2_result': {'phase_id': 'phase_2_template_rendering', 'status': 'success'},
        ...
        'phase_5_result': {
            ...
            'status': 'success',
            'health_checks': {
                'containers': {'total': 3, 'healthy': 3, 'unhealthy': 0}
            }
        },
