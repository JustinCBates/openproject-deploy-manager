...
class CleanupHandler:
    ...
    def __init__(self, config: Optional[Dict[str, Any]] = None):
        ...
        self.protected_dirs = set(
            self.config.get("protected_dirs", self.PROTECTED_PATTERNS)
        )
    ...
    def cleanup(
        ...
        try:
            ...
            logger.info(
                f"Cleanup complete: {files_removed} files, {dirs_removed} dirs, "
    ...
    def cleanup_old_artifacts(
        self, artifacts_dir: Path, keep_count: int = 5, pattern: str = "*"
    ) -> CleanupResult:
        """
        ...
        try:
            ...
            if not to_remove:
                logger.info(
                    f"No old artifacts to clean ({len(artifacts)} total, keeping {keep_count})"
                )
                return CleanupResult(
                    success=True,
                    files_removed=0,
            ...
            logger.info(
                f"Cleaning {len(to_remove)} old artifacts (keeping {keep_count} newest)"
            )
    ...
    def _find_matches(
        self, directory: Path, patterns: List[str], recursive: bool
    ) -> Set[Path]:
...
def main():
    ...
    with tempfile.TemporaryDirectory() as tmpdir:
        ...
        logger.info(f"Files before cleanup: {list(test_dir.rglob('*'))}\n")

        # Test cleanup
        result = handler.cleanup(
            test_dir, patterns=["*.tmp", "*.log", "__pycache__"], recursive=True
        )

        logger.info(f"\nCleanup Result:")
        logger.info(f"  Success: {result.success}")
        logger.info(f"  Files removed: {result.files_removed}")
        logger.info(f"  Directories removed: {result.directories_removed}")
        logger.info(f"  Space freed: {result.space_freed_bytes} bytes")
