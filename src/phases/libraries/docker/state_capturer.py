...
class StateCapturer:
    ...
    def capture(self, project_name: Optional[str] = None) -> CaptureResult:
        ...
        try:
            ...
            logger.info(f"✅ Captured state: {len(containers)} containers, "
    ...
    def _capture_containers(self, project_name: Optional[str]) -> List[ContainerState]:
        ...
        try:
            ...
            if project_name:
                cmd.extend(['--filter', f'label=com.docker.compose.project={project_name}'])
    ...
    def _get_container_details(self, container_id: str) -> Dict[str, Any]:
        ...
        try:
            ...
            details['volumes'] = [
                ...
                for m in mounts if m.get('Source') and m.get('Destination')
            ]
            ...
            details['command'] = ' '.join(container.get('Config', {}).get('Cmd', []) or [])
            details['entrypoint'] = ' '.join(container.get('Config', {}).get('Entrypoint', []) or [])
...
def main():
    ...
    if result.success:
        print(f"   ✅ Captured {len(result.containers)} containers for project 'test-deployment'")
