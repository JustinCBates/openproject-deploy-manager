...
class ConfigBackupper:
    ...
    def backup(self, files: List[Path], output_dir: Optional[Path] = None) -> BackupResult:
        """
        ...
        try:
            ...
            for file_path in files:
                ...
                logger.debug(f"  Backed up: {file_path} -> {backup_path} ({file_size} bytes)")
            ...
            logger.info(f"✅ Backed up {len(backed_up_files)} file(s) ({total_size} bytes)")
    ...
    def backup_directory(self, source_dir: Path, output_dir: Optional[Path] = None) -> BackupResult:
        """
        ...
        try:
            ...
            logger.info(f"✅ Backed up directory: {file_count} files ({total_size} bytes)")
