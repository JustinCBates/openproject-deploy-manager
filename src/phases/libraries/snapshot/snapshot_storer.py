...
class SnapshotStorer:
    ...
    def store(self, snapshot: Snapshot) -> StoreResult:
        ...
        try:
            ...
            return StoreResult(
                success=True, snapshot_id=snapshot.id, snapshot_path=snapshot_file
            )
        ...
        try:
            ...
            for snapshot_file in snapshot_files:
                try:
                    ...
                    snapshot.metadata["container_count"] = len(
                        snapshot_dict.get("containers", [])
                    )
                    snapshot.metadata["network_count"] = len(
                        snapshot_dict.get("networks", [])
                    )
                    snapshot.metadata["volume_count"] = len(
                        snapshot_dict.get("volumes", [])
                    )
    ...
    def get_latest(self, project_name: Optional[str] = None) -> LoadResult:
        ...
        if not snapshots:
            return LoadResult(success=False, error="No snapshots found")
...
def main():
    ...
    with tempfile.TemporaryDirectory() as temp_dir:
        ...
        snapshot = Snapshot(
            ...
            volumes=[{"name": "test-volume", "driver": "local"}],
