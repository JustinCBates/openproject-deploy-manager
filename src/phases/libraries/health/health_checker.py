...
class HealthChecker:
    ...
    def wait_for_healthy(self, services: List[str], timeout: int = 60) -> Result:
        ...
        """
        logger.info(f"Waiting for {len(services)} services to become healthy (timeout: {timeout}s)")
        ...
        while True:
            ...
            logger.debug(f"Health check: {healthy_count}/{len(services)} healthy ({elapsed:.1f}s)")
