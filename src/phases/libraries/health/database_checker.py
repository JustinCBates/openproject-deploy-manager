...
class DatabaseChecker:
    ...
    def check(self, host: str, port: Optional[int] = None, db_type: str = 'postgres', 
              timeout: Optional[int] = None) -> DbCheckResult:
        """
        ...
        try:
            ...
            if result == 0:
                logger.info(f"Database reachable: {db_type} at {host}:{port} ({response_time:.2f}ms)")
            ...
            logger.warning(f"Database connection timeout: {host}:{port} after {timeout_seconds}s")
            return DbCheckResult(
                host=host,
                port=port,
                db_type=db_type,
    ...
    def check_postgres(self, host: str, port: int = 5432, timeout: Optional[int] = None) -> DbCheckResult:
        """
    ...
    def check_mysql(self, host: str, port: int = 3306, timeout: Optional[int] = None) -> DbCheckResult:
        """
    ...
    def check_mongodb(self, host: str, port: int = 27017, timeout: Optional[int] = None) -> DbCheckResult:
        """
...
def main():
    ...
    for host, port, db_type in tests:
        ...
        print(f"{status_icon} {db_type}@{host}:{port}: {result.status.value} ({result.response_time_ms:.2f}ms)")
