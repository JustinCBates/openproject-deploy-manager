...
class ConnectivityTester:
    ...
    def test_connection(self, host: str, port: int, protocol: str = 'tcp', 
                       timeout: Optional[int] = None) -> Tuple[bool, float, Optional[str]]:
        """
        ...
        try:
            ...
            if protocol.lower() == 'tcp':
                ...
                if result == 0:
                    ...
                    sock.close()
                    return False, response_time, f"Connection refused (error code: {result})"
            else:
                ...
                try:
                    ...
                    return True, response_time, "UDP (no response, but sent successfully)"
    ...
    def test(self, tests: List[ConnectivityTest]) -> ConnectivityReport:
        ...
        for test in tests:
            ...
            logger.info(f"{status_icon} {test.source} → {test.target_host}:{test.target_port} "
        ...
        logger.info(f"Connectivity test complete: {successful}/{len(results)} connections successful")
...
def main():
    ...
    for result in report.tests:
        ...
        print(f"{status_icon} {result.test.source} → {result.test.target_host}:{result.test.target_port}: "
