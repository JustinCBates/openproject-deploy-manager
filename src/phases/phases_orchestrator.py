...
from phases.phase_1_preflight.phase_1_preflight_orchestrator import (
    Phase1PreflightOrchestrator,
)
from phases.phase_2_template_rendering.phase_2_template_rendering_orchestrator import (
    Phase2TemplateRenderingOrchestrator,
)
from phases.phase_3_snapshot.phase_3_snapshot_orchestrator import (
    Phase3SnapshotOrchestrator,
)
from phases.phase_4_deployment.phase_4_deployment_orchestrator import (
    Phase4DeploymentOrchestrator,
)
from phases.phase_5_health_verification.phase_5_health_verification_orchestrator import (
    Phase5HealthVerificationOrchestrator,
)
from phases.phase_6_post_deployment.phase_6_post_deployment_orchestrator import (
    Phase6PostDeploymentOrchestrator,
)
...
class PhasesOrchestrator:
    ...
    def __init__(self, project_root: Path, config: Dict[str, Any]):
        ...
        self.phase_2_template_rendering = Phase2TemplateRenderingOrchestrator(
            project_root, config
        )
        ...
        self.phase_5_health_verification = Phase5HealthVerificationOrchestrator(
            project_root, config
        )
        self.phase_6_post_deployment = Phase6PostDeploymentOrchestrator(
            project_root, config
        )

    def execute_main_deployment_flow(
        ...
        try:
            ...
            return {"status": "success", "phases": results, "context": context}
            ...
            return {"status": "error", "error": str(e), "phases": results}

    def execute_rollback_flow(
        self, snapshot_id: str, reason: str = "Manual rollback"
    ) -> Dict[str, Any]:
        """
        ...
        try:
            ...
            if compose_file and Path(compose_file).exists():
                executor = ComposeExecutor(
                    config={"compose_file": compose_file, "project_name": project_name}
                )
                    ...
                    for config_file_name in snapshot.config_files:
                        ...
                        if source.exists():
                            try:
                                ...
                                logger.error(
                                    f"   ❌ Failed to restore {config_file_name}: {e}"
                                )
                                rollback_result["errors"].append(
                                    f"Failed to restore {config_file_name}"
                                )
                        else:
                            logger.warning(
                                f"   ⚠️  Backup not found: {config_file_name}"
                            )

                    logger.info(
                        f"✅ Restored {restored_count}/{len(snapshot.config_files)} configuration files"
                    )
            ...
            health_context = {"config": self.config, "project_name": project_name}
            ...
            failed_steps = [
                k for k, v in rollback_result["steps"].items() if v == "failed"
            ]
