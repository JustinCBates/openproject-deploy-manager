...
@click.option(
    "--skip-health", is_flag=True, help="Skip health verification (faster, less safe)"
)
@click.option(
    "--cleanup",
    is_flag=True,
...
def deploy(
    ...
    if cleanup and not dry_run:
        ...
        if compose_file:
            ...
            compose_art = artifacts.get("compose_file") or (
                artifacts.get("compose_result") or {}
            ).get("compose_file")
            if compose_art:
        ...
        if config_data.get("project_name"):
            ...
            phase4 = result.get("phases", {}).get("phase_4_deployment", {})
            artifacts = phase4.get("artifacts", {}) if isinstance(phase4, dict) else {}
            project_name = artifacts.get("project_name") or config_data.get(
                "project_name"
            )
        ...
        if compose_path:
            ...
            executor = ComposeExecutor(
                config={"compose_file": str(compose_path), "project_name": project_name}
            )
            down_result = executor.down(volumes=False, remove_orphans=True)

            if down_result.success:
                click.echo(
                    f"✅ Cleanup complete: stopped deployment (project={project_name or 'unknown'})"
                )
            else:
                click.echo(
                    f"⚠️  Cleanup failed: {down_result.error or down_result.stderr}"
                )
        else:
            click.echo(
                "⚠️  Cleanup requested but no compose file found; skipping docker compose down"
            )
...
def rollback(
    ...
    if not yes:
        if not click.confirm(
            "⚠️  This will stop the current deployment and rollback. Continue?"
        ):
...
def snapshot_list(limit: int, json_format: bool):
    ...
    """
    snapshot_dir = (
        project_root / "runtime" / "phase_3_snapshot" / "outputs" / "snapshots"
    )
    ...
    if history_file.exists():
        ...
        if json_format:
            ...
            for entry in entries:
                ...
                status = entry.get("deployment_status") or entry.get(
                    "status", "unknown"
                )
